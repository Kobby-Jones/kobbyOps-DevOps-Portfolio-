...
export function parseContentPayload(input: Record<string, unknown>) {
  ...
  return {
    ...
    cover_asset_id: cleanText(input.coverAssetId, 80) || null,
...
export function parseResourcePayload(input: Record<string, unknown>) {
  ...
  return {
    ...
    thumbnail_asset_id: cleanText(input.thumbnailAssetId, 80) || null,
    file_url: cleanText(input.fileUrl, 1000) || null,
    file_asset_id: cleanText(input.fileAssetId, 80) || null,
    prepared_resource_id: cleanText(input.preparedResourceId, 80) || null,
    has_download: Boolean(cleanText(input.fileAssetId, 80) || cleanText(input.fileUrl, 1000)),
