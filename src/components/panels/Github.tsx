...
export default function GithubPanel() {
  ...
  if (loading) return <div className="text-zinc-400 text-sm">Loading GitHub data…</div>;
  if (!data || (data as any).error)
    return (
      <div className="text-zinc-400 text-sm">
        Failed to load GitHub data. Ensure <code>GITHUB_USERNAME</code> and <code>GITHUB_TOKEN</code> are set.
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Stat icon={<Code2 size={14} className="text-teal-400" />} label="Repositories" value={metrics.repoCount} sub={`${user.public_repos} public`} />
        <Stat icon={<Star size={14} className="text-teal-400" />} label="Total Stars" value={metrics.totalStars} />
        <Stat icon={<GitFork size={14} className="text-teal-400" />} label="Total Forks" value={metrics.totalForks} />
        <Stat icon={<Users size={14} className="text-teal-400" />} label="Followers" value={user.followers} sub={`${user.following} following`} />
      </div>

      {/* Repos list */}
      <div>
        <div className="text-zinc-300 mb-2 font-medium">Repositories (sorted by last update)</div>
          ...
          {repos
            ...
            .map((r) => (
              ...
              >
                ...
                {r.description && <div className="text-sm text-zinc-400 mt-1">{r.description}</div>}
                ...
                Updated {new Date(r.pushed_at).toLocaleDateString("en-US", {
                    month: "short",
                    day: "numeric",
                })}
                </span>
                ...
                {r.homepage && (
                  <div className="mt-2 text-right">
                    <a href={r.homepage} target="_blank" rel="noreferrer" className="text-teal-300 text-xs hover:underline flex items-center gap-1 justify-end">
                      <Globe size={12} />
                      Live
                    </a>
                  </div>
